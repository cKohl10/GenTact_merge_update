...
class AlligatorSaveOperator(Operator, ExportHelper):
    ...
    def check_children_for_sensors(self, obj, parent_path, folder_path):
        ...
        for child in obj.children:
            ...
            child_sensor_data = self.check_children_for_sensors(child, parent_path, folder_path)
            ...
            self.export_object(child, folder_path + "/" + child.name + ".stl")
            ...
            if alligator_clip_attribute_name in mesh.attributes:
                ...
                for i in range(len(is_clip_data)):
                    if is_clip_data[i].value:
                        clip_pos.append(SensorData(mesh.attributes[pos_attribute_name].data[i].vector, 0.1, parent_path, True, name=child.name))
            ...
            for i in range(len(pos_attribute_data)):
                if is_sensor_data[i].value:
                    if default_radius:
                        ...
                        child_sensor_data.append(SensorData(pos_attribute_data[i].vector, rad_attribute_data[i].value, parent_path, name=child.name))
    ...
    def save_attribute_to_csv(self, context, folder_path):
        # Get the object
        obj = context.object
        ...
        sensor_data = self.check_children_for_sensors(obj, "", mesh_folder_path)
        ...
        with open(folder_path + '/sensor_config.csv', 'w', newline='') as csvfile:
            ...
            for element in sensor_data:
                ...
                if element.is_clip:
                    ...
                    csv_writer.writerow([index_counter, pos.x * self.unit_scale, pos.y * self.unit_scale, pos.z * self.unit_scale, element.radius, element.parent])
    ...
    def export_object(self, ob, file_path):
        ...
        bpy.ops.wm.stl_export(filepath=file_path, export_selected_objects=True, global_scale=self.unit_scale)
...
class SensorData:
    def __init__(self, pos, radius, parent, is_clip=False, name=""):
        self.pos = pos
        self.radius = radius
        self.parent = parent
